...
test("display of normal difficulty results when click on button normal", async () => {
  ...
  const findNormalButton = () =>
    screen.findByRole("button", { name: "NORMAL" });
...
test("display of all difficulty results after click on hard button then on all button", async () => {
  render(<Home />);
  const allResults = () => screen.findAllByTestId("gridRow");
  const numberOfResults = await allResults();
  expect(numberOfResults).toHaveLength(8);
...
test("display scores of user-test when click on button See my results", async () => {
  render(<Home />);
  ...
  const getPlayerResultsButton = () =>
    screen.findByRole("button", { name: "See my results" });
